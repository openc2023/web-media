...
function drawMiniVisualizer(audioLevel) {
  ...
  // 性能优化：使用缓存的渐变
  if (!drawMiniVisualizer.gradients) {
    drawMiniVisualizer.gradients = [];
    drawMiniVisualizer.glowGradients = [];
    
    // 创建颜色渐变
    for (let l = 0; l < lineCount; l++) {
      // 温度 - 红橙渐变
      if (l === 0) {
        const tempGradient = miniCtx.createLinearGradient(0, 0, miniW, 0);
        tempGradient.addColorStop(0, 'rgba(255, 120, 120, 0.9)');
        tempGradient.addColorStop(0.5, 'rgba(255, 200, 120, 0.9)');
        tempGradient.addColorStop(1, 'rgba(255, 150, 150, 0.9)');
        drawMiniVisualizer.gradients.push(tempGradient);
        
        const tempGlowGradient = miniCtx.createLinearGradient(0, 0, miniW, 0);
        tempGlowGradient.addColorStop(0, 'rgba(255, 120, 120, 0.4)');
        tempGlowGradient.addColorStop(0.5, 'rgba(255, 200, 120, 0.4)');
        tempGlowGradient.addColorStop(1, 'rgba(255, 150, 150, 0.4)');
        drawMiniVisualizer.glowGradients.push(tempGlowGradient);
      }
      // 湿度 - 蓝青渐变
      else if (l === 1) {
        const humidityGradient = miniCtx.createLinearGradient(0, 0, miniW, 0);
        humidityGradient.addColorStop(0, 'rgba(120, 150, 255, 0.8)');
        humidityGradient.addColorStop(0.5, 'rgba(120, 255, 255, 0.8)');
        humidityGradient.addColorStop(1, 'rgba(150, 180, 255, 0.8)');
        drawMiniVisualizer.gradients.push(humidityGradient);
        
        const humidityGlowGradient = miniCtx.createLinearGradient(0, 0, miniW, 0);
        humidityGlowGradient.addColorStop(0, 'rgba(120, 150, 255, 0.3)');
        humidityGlowGradient.addColorStop(0.5, 'rgba(120, 255, 255, 0.3)');
        humidityGlowGradient.addColorStop(1, 'rgba(150, 180, 255, 0.3)');
        drawMiniVisualizer.glowGradients.push(humidityGlowGradient);
      }
      // 风速 - 绿黄渐变
      else if (l === 2) {
        const windGradient = miniCtx.createLinearGradient(0, 0, miniW, 0);
        windGradient.addColorStop(0, 'rgba(120, 255, 120, 0.7)');
        windGradient.addColorStop(0.5, 'rgba(255, 255, 120, 0.7)');
        windGradient.addColorStop(1, 'rgba(150, 255, 180, 0.7)');
        drawMiniVisualizer.gradients.push(windGradient);
        
        const windGlowGradient = miniCtx.createLinearGradient(0, 0, miniW, 0);
        windGlowGradient.addColorStop(0, 'rgba(120, 255, 120, 0.3)');
        windGlowGradient.addColorStop(0.5, 'rgba(255, 255, 120, 0.3)');
        windGlowGradient.addColorStop(1, 'rgba(150, 255, 180, 0.3)');
        drawMiniVisualizer.glowGradients.push(windGlowGradient);
      }
    }
  }
  
  const lineWidths = [2, 1.5, 1.2]; // 线条宽度
  const maxLineHeight = miniH * 0.7; // 减少最大线条高度，留出更多空间
  ...
  // 绘制三条天气数据线条
  for (let l = 0; l < lineCount; l++) {
    const lineGradient = drawMiniVisualizer.gradients[l];
    const glowGradient = drawMiniVisualizer.glowGradients[l];
    const lineWidth = lineWidths[l];
    const yOffset = (lineCount - l - 1) * lineSpacing; // 线条垂直偏移
    
    // 优化上下文状态设置
    miniCtx.shadowColor = glowGradient;
    miniCtx.shadowBlur = 3; // 减少模糊效果提高性能
    miniCtx.strokeStyle = lineGradient;
  ...
  // 优化文本渲染
  miniCtx.font = '8px Arial'; // 字体大小
  miniCtx.textAlign = 'center';
  
  // 性能优化：使用缓存的文本渐变
  if (!drawMiniVisualizer.textGradients) {
    drawMiniVisualizer.textGradients = {};
    
    // 温度文本渐变
    const tempGradient = miniCtx.createLinearGradient(miniW * 0.2 - 30, 0, miniW * 0.2 + 30, 0);
    tempGradient.addColorStop(0, 'rgba(255, 120, 120, 0.9)');
    tempGradient.addColorStop(0.5, 'rgba(255, 200, 120, 0.9)');
    tempGradient.addColorStop(1, 'rgba(255, 150, 150, 0.9)');
    drawMiniVisualizer.textGradients.temp = tempGradient;
    
    // 湿度文本渐变
    const humidityGradient = miniCtx.createLinearGradient(miniW * 0.5 - 30, 0, miniW * 0.5 + 30, 0);
    humidityGradient.addColorStop(0, 'rgba(120, 150, 255, 0.8)');
    humidityGradient.addColorStop(0.5, 'rgba(120, 255, 255, 0.8)');
    humidityGradient.addColorStop(1, 'rgba(150, 180, 255, 0.8)');
    drawMiniVisualizer.textGradients.humidity = humidityGradient;
    
    // 风速文本渐变
    const windGradient = miniCtx.createLinearGradient(miniW * 0.8 - 30, 0, miniW * 0.8 + 30, 0);
    windGradient.addColorStop(0, 'rgba(120, 255, 120, 0.7)');
    windGradient.addColorStop(0.5, 'rgba(255, 255, 120, 0.7)');
    windGradient.addColorStop(1, 'rgba(150, 255, 180, 0.7)');
    drawMiniVisualizer.textGradients.wind = windGradient;
  }
  
  // 显示温度 - 调整垂直位置确保完全显示
  miniCtx.fillStyle = drawMiniVisualizer.textGradients.temp;
  miniCtx.fillText(`TEMP: ${busanWeatherData.temperature}°C`, miniW * 0.2, maxLineHeight * 1.2);
  
  // 显示湿度 - 调整垂直位置确保完全显示
  miniCtx.fillStyle = drawMiniVisualizer.textGradients.humidity;
  miniCtx.fillText(`HUM: ${busanWeatherData.humidity}%`, miniW * 0.5, maxLineHeight * 1.2);
  
  // 显示风速 - 调整垂直位置确保完全显示
  miniCtx.fillStyle = drawMiniVisualizer.textGradients.wind;
  miniCtx.fillText(`WIND: ${busanWeatherData.windSpeed}km/h`, miniW * 0.8, maxLineHeight * 1.2);
...
function drawLine(line, audioLevel){
  ...
  // 为该线条生成唯一的颜色渐变基色
  const hueShift = (line.born * 50) % 360; // 基于出生时间的色调偏移
  const baseHue = (hueShift + audioLevel * 120) % 360; // 结合音频影响
  ...
  if(L > 0 && tailLen > 0){
    ...
    for(let k=1; k<=L; k++){
      ...
      // 根据位置和音频电平计算渐变色彩
      const tailHue = (baseHue + tRel * 60) % 360; // 尾部色调渐变
      const tailSaturation = 70 + audioLevel * 20; // 随音频变化的饱和度
      const tailLightness = 60 + tRel * 20; // 尾部亮度渐变
      const tailColor = `hsla(${tailHue}, ${tailSaturation}%, ${tailLightness}%, 1)`;
  ...
  // —— 再画主线（使用颜色渐变）——
  const gradient = ctx.createLinearGradient(0, 0, W, 0); // 水平渐变
  
  // 根据音频电平调整渐变颜色
  const startHue = (baseHue - 30) % 360;
  const endHue = (baseHue + 30) % 360;
  const saturation = 80 + audioLevel * 15;
  const lightness = 70;
  
  gradient.addColorStop(0, `hsla(${startHue}, ${saturation}%, ${lightness}%, 0.8)`);
  gradient.addColorStop(0.5, `hsla(${baseHue}, ${saturation + 10}%, ${lightness + 5}%, 0.9)`);
  gradient.addColorStop(1, `hsla(${endHue}, ${saturation}%, ${lightness}%, 0.8)`);
  ...
  ctx.shadowColor = `hsla(${baseHue}, 70%, 80%, 0.7)`;
